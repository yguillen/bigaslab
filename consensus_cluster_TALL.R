...
#subset 27 patients
tallml_rem<-sample(tallml_rem, 27)
dim(tallml_rem)

tallml_rel<-tallcount[,colnames(tallcount) %in% metadata[metadata$GSEA=="TARGET" & metadata$First_event!="Remission" & metadata$First_event!="NA" & metadata$First_event!="Censored",]$SampID]
dim(tallml_rel)[2]
#27 patients no remission, no NA, no censored
...
# For selecting top 100 highest variance
...
# For selecting top 100 highest variance
vars <- sort(apply(tallml, 1, var, na.rm = TRUE), decreasing = TRUE)
summary(vars[1:100])
data <- tallml[names(vars)[1:100], ]
dim(data)

## Using genes up and down Remission vs. non_Remission from Clinical_TCell.R script
length(c(genesup,genesdown))
data <- tallml[c(genesup,genesdown), ]
dim(data)
