...
# Code instructions for bglab package: 
#https://github.com/wjawaid/bglab

# Code instructions for scran, including cyclone
#https://rdrr.io/bioc/scran/f/vignettes/scran.Rmd

# Code instructions for simple scRNAseq (Marioni lab)
#https://rdrr.io/github/MarioniLab/simpleSingleCell/f/vignettes/intro.Rmd



### Sample code gastrulation paper Gotggens lab ## 

# Load the library, download counts and metadata 
# from http://gastrulation.stemcells.cam.ac.uk. 

# To save project
#setwd("/Users/yguillen/Desktop/temp/Rprojects/")

# Set working directory
#setwd("/Volumes/grcmc/YGUILLEN/scRNA_cam/Roshana_scRNA")
setwd("/Users/yguillen/Desktop/temp/scRNA_cam/Roshana_scRNA")

# Download data if following tutorial
#system("curl -O http://gastrulation.stemcells.cam.ac.uk/data/counts.tar.gz")
#untar("counts.tar.gz")

#counts<-read.table("counts.txt",header=TRUE,row.names=1,sep=" ",stringsAsFactors = FALSE)
counts<-read.table("HtseqCounts_GL53.txt",header=TRUE,row.names=1,stringsAsFactors = FALSE)
...
# Download metadata
#system("curl -O http://gastrulation.stemcells.cam.ac.uk/data/metadataAll.txt")
#meta <- read.table("http://gastrulation.stemcells.cam.ac.uk/data/metadataAll.txt",header=TRUE,stringsAsFactors = FALSE)

# Or upload from AGM Zaki Results

# meta after his quality control
metazaki<-read.table("../../Roshana_scRNA/INDEX_data/merged_files.txt",sep=",",header = TRUE)
row.names(metazaki)<-metazaki$sampleName
row.names(metazaki)<-gsub('_R._....fastq','',row.names(metazaki))
row.names(metazaki)<-gsub('GL53_','C',row.names(metazaki))

metazaki_clusters<-subset(metazaki,select=c(Cluster))
...
#meta<-read.delim("/Volumes/grcmc/YGUILLEN/scRNA_cam/Roshana_scRNA/INDEX_data/All_plates_comma.tsv",sep="\t",stringsAsFactors = FALSE)
meta<-read.delim("/Users/yguillen/Desktop/temp/scRNA_cam/Roshana_scRNA/INDEX_data/All_plates_comma.tsv",sep="\t",stringsAsFactors = FALSE)
dim(meta)
row.names(meta)<-meta$Plate_Well

# info meta in counts not present in allsamples (before Zaki filter).
# Create variables from meta -- meta2
meta2<-as.data.frame(colnames(counts))
colnames(meta2)<-"sampleID"
rownames(meta2)<-meta2$sampleID
meta2$population<-gsub('C[0-9].*Gfi','Gfi',meta2$sampleID)
meta2$population<-gsub('_S[1-9].*','',meta2$population)
meta2$Plate<-gsub('^.*Plate','Plate',meta2$sampleID)
meta2$Plate<-gsub('_.*','',meta2$Plate)
meta2$position<-gsub('^.*Plate._','',meta2$sampleID)
meta2$position<-gsub('_.*','',meta2$position)
meta2$Plate_Well<-paste(meta2$Plate,toupper(meta2$position),sep="_")
dim(meta2)


# merge meta and meta2, need a variable called flowCell for QC in pipeline
dim(meta)
meta<-merge(meta2,meta,by="Plate_Well",all.x=TRUE)
row.names(meta)<-meta$sampleID
meta$flowCell<-paste("SRX",meta$Plate,sep=".")
meta$flowCell<-gsub('Plate','',meta$flowCell)

colnames(meta)<-gsub('All.Events.','',colnames(meta))
colnames(meta)<-gsub('\\.\\w\\w.*','',colnames(meta))
...
# transform negative values into positive for markers with values < 0
# min for marker value
sumDll4<-min((meta[!is.na(meta$Dll4),])$Dll4)
sumNotch2<-min((meta[!is.na(meta$Notch2),])$Notch2)
sumCD45<-min((meta[!is.na(meta$CD45),])$CD45)
sumcKIT<-min((meta[!is.na(meta$cKIT),])$cKIT)
sumGfi1<-min((meta[!is.na(meta$Gfi1),])$Gfi1)
sumCD41<-min((meta[!is.na(meta$CD41),])$CD41)
sumCD41<-min((meta[!is.na(meta$Jag1),])$Jag1)

# For scanpy output, try not not transform...
#meta$Dll4<-meta$Dll4+abs(sumDll4)
#meta$Notch2<-meta$Notch2+abs(sumNotch2)
#meta$CD45<-meta$CD45+abs(sumCD45)
#meta$cKIT<-meta$cKIT+abs(sumcKIT)
#meta$Gfi1<-meta$Gfi1+abs(sumGfi1)
#meta$CD41<-meta$CD41+abs(sumCD41)

meta<- meta[order(meta$sampleID),]

# create variable ratio ligand/receptor in terms of markers
# variable with ratio ligands and ratio receptors
#meta$lig<-meta$Jag1/(meta$Jag1+meta$Dll4)
#meta$rec<-meta$Notch1/(meta$Notch2+meta$Notch1)


#in scanpy, we should avoid the headers
write.table(meta,"/Volumes/grcmc/YGUILLEN/scRNA_cam/scanpy_run/metadata_scRNAseq_R2.txt",sep="\t",col.names = FALSE,row.names = TRUE,quote = FALSE)

# Load gtf used by Zaki instead of using missing ensembl version from biomart. For next
# analysis, download from biomart (see below)
gtf<-read.delim("simple_gtf.txt",header = FALSE)
gtf<-gtf[,c(2,3)]
geneTable<-gtf
...
# Download the appropiate gene and ensembl IDs from ensembl
library(biomaRt)
# Check host if necessary, add then in useMart option host=host
#host <- "oct2014.archive.ensembl.org"   # Ensembl 76
#ensembl <- useMart(biomart = "ENSEMBL_MART_ENSEMBL",verbose = TRUE)
ensembl<-useMart("ensembl", dataset="mmusculus_gene_ensembl")

# listAttributes(ensembl) to inspect attributes.

# Now, retrieve the biomart data
#listgenes=genecounts
...
# ERCCs: Quality control RNA quantification. Measure of sensitivity and specificity.

#number of rows with QC information from HTSeq file
# The last 10 rows of HTSeq counts are for QC control
# There are 92 RNA spike-in, located at the end of the counts file
qcInd <- grep ("^_",rownames(counts))
...
cyc<-cyclone(expall,mm.pairs)

cyctab<-as.data.frame(cyc$phases)
row.names(cyctab)<-colnames(expall)
colnames(cyctab)<-"Phase"

cycG1<-as.data.frame(cyc$scores$G1)
row.names(cycG1)<-colnames(expall)
colnames(cycG1)<-"G1"

cycG2M<-as.data.frame(cyc$scores$G2M)
row.names(cycG2M)<-colnames(expall)
colnames(cycG2M)<-"G2M"

cycS<-as.data.frame(cyc$scores$S)
row.names(cycS)<-colnames(expall)
colnames(cycS)<-"S"

write.table(cyctab,"scanpy_data/cyclone_scRNAseq_R_Elaine.txt",sep="\t",col.names = FALSE,row.names = TRUE,quote = FALSE)
write.table(cycG1,"scanpy_data/G1_score_cyclone_Elaine.txt",sep="\t",col.names = FALSE,row.names = TRUE,quote = FALSE)
write.table(cycG2M,"scanpy_data/G2M_score_cyclone_Elaine.txt",sep="\t",col.names = FALSE,row.names = TRUE,quote = FALSE)
write.table(cycS,"scanpy_data/S_score_cyclone_Elaine.txt",sep="\t",col.names = FALSE,row.names = TRUE,quote = FALSE)


scd <- prcomp(assay(sce,"logcounts"),scale.= TRUE)
plot(scd, reduceMethod = "pca", colorBy = "population", 
     legPos = "topright", outline = "black")


# tSNe It only works with use_dist=FALSE
scd <- runTSNE(scd, ndims = 2, seed = 0, 
               verbose = TRUE, use_dist = FALSE)

plot(scd, reduceMethod = "tsne", colorBy = "population", 
     legPos = "bottomleft", outline = "black")
dev.off()

# Difusion map
pdf(file = "../Roshana_scRNA/Plots/bglab_R/difusion_map.pdf")
dmap <- diffuseMat2(exprs(scd))
plot(dmap$vectors, bg = ggCol(pData(scd)[,"population"]), col = "black", pch = 21)
dev.off()

#UMAP
library(umap)
# 50 pcas
scUMAP<-umap(scd@pca@eigenvectors[,1:50])
head(scUMAP$data)

# 3D plot UMAP
library(plotly)

UMAPdata<-as.data.frame(scUMAP$data)
UMAPlayout<-as.data.frame(scUMAP$layout)


UMAPdata$pop<-gsub('.*_Gfi','Gfi',rownames(UMAPdata))
UMAPdata$pop<-gsub('_S[0-9]*','',UMAPdata$pop)

table(UMAPdata$pop)

metaUMAP<-merge(meta,UMAPdata,by="row.names",all=TRUE)
rownames(metaUMAP)<-metaUMAP$Row.names
metaUMAP$Row.names=NULL


p <- plot_ly(metaUMAP, x = ~PC1, y = ~PC2, z = ~PC3,color=~population) %>%
  add_markers() %>%
...
ggplot(data=metaUMAP,aes(x=PC1,y=PC2))+
...
colnames(metaUMAP)
metaUMAP<-metaUMAP[,c(1:27)]



# merge with clusters info zaki
metaUMAP<-merge(metazaki_clusters,metaUMAP,by="row.names",all.y=TRUE)
row.names(metaUMAP)<-metaUMAP$sampleID

ggplot(metaUMAP,aes(x=PC1,y=PC2))+
  geom_point(aes(color=Cluster))+
  #scale_colour_gradient(low = "white", high = "red")+
  theme_bw()+
  theme(legend.position = "bottom")

p <- plot_ly(metaUMAP, x = ~PC1, y = ~PC2, z = ~PC3,color=~Cluster) %>%
  add_markers() %>%
  layout(scene = list(xaxis = list(title = 'UMAP_C1'),
                      yaxis = list(title = 'UMAP_C2'),
                      zaxis = list(title = 'UMAP_C3'))
...
## Following bglab package, cluster results ressemble quite well those obtained by Zaki

# Correlation cell surface markers and counts
genemark<-c("Dll4","Notch2","Kit","Gfi1","Itga2b",
            "Notch1","Pecam1","Jag1","Ptprc",
            "Adgrg1","Gfi1b")
tabgenes<-geneTable[geneTable$Associated_Gene_Name %in% genemark,]
...
transdata<-expallt[,colnames(expallt) %in% as.character(tabgenes$Ensembl_Gene_ID)]
colnames(transdata)
rownames(transdata)
rownames(meta)

transmet<-merge(metaUMAP,transdata,by="row.names")
rownames(transmet)<-transmet$Row.names
transmet$Row.names=NULL

#BiocManager::install("ggnewscale")
library(ggnewscale)

combgenes<-ggplot(transmet,aes(x=PC1,y=PC2))+
  geom_point(alpha=1,size=5,color="grey")+
  new_scale("color")+
  geom_point(aes(color=ENSMUSG00000027276.7),alpha=1,size=4)+
  scale_colour_gradient(low = "white", high = "red")+
  new_scale("color")+
  geom_point(aes(color=ENSMUSG00000026923.15),alpha=1,size=2)+
  scale_colour_gradient(low = "white", high = "blue")+
  theme_bw()+
  theme(legend.position = "bottom")
combgenes
ggsave(combgenes,file="../Roshana_scRNA/Plots/bglab_R/combgenes_Jag1_Notch1.pdf")
